...
def runTrial(nrWordsPerSentence =7,nrSentences=3,nrSteeringMovementsWhenSteering=2, interleaving="word"): 
    ...
    if(interleaving == "word"):
        ...
        for i in range(nrSentences):
            for j in range(nrWordsPerSentence):
                # Calculate what the typing time would have been for this word j, and update the total trialTime 
                typingTime = 0
                if j == 0:     
                    typingTime += retrievalTimeSentence   
                typingTime += timePerWord
        ...
        for i in range(nrSentences):
            for j in range(nrWordsPerSentence):
                ...
                # After the word is typed, you check whether the threshold is reached (only if it's not the last word of the last sentence) if it is, you update the vehicle location using Active Steering.
...
def runSimulations(nrSims = 100):
    ...
    for i in (interleaving):
        ...
        for j in range (nrSims):
            ...
            tot_time, mean_dev, max_dev = runTrial(nrWordsPerSentence, 10, 4, "bonus")
            totalTime.append(tot_time)
            meanDeviation.append(mean_dev)
            maxDeviation.append(max_dev)
            Condition.append(i)
            print("Simulation ", j, " of ", nrSims, " completed for ", i, " interleaving.")
    

    data = pd.DataFrame({'Total Time': totalTime, 'Mean Deviation': meanDeviation, 'Max Deviation': maxDeviation, })
    plt.figure(figsize=(10, 7))
    sns.scatterplot(x="Total Time", y="Max Deviation", data=data,)
